...
async function handleScreenshot(context, params) {
    const action = async () => {
        try {
            ...
            const { data } = await page.sendCDP("Page.captureScreenshot", {
                format: "png",
                fromSurface: true,
            });
            ...
            if (metadata.width && metadata.height) {
                ...
                const shrink = Math.min(1568 / metadata.width, 1568 / metadata.height, Math.sqrt((1.15 * 1024 * 1024) / pixels));
                ...
                if (shrink < 1) {
                    ...
                    process.stderr.write(`[Screenshot] Scaling image from ${metadata.width}x${metadata.height} (${(pixels / (1024 * 1024)).toFixed(2)}MP) to ${newWidth}x${newHeight} (${((newWidth * newHeight) / (1024 * 1024)).toFixed(2)}MP) for Claude vision API\n`);
